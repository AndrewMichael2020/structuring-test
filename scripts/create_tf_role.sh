...
EOF
  ...
  gcloud iam roles create "${ROLE_ID}" --project "${PROJECT_ID}" \
    ...
    --description "Minimal permissions for the structuring-test Terraform configuration" \
    --stage "GA" \
    --permissions "serviceusage.services.enable,storage.buckets.create,storage.buckets.get,storage.buckets.update,storage.buckets.setIamPolicy,artifactregistry.repositories.create,artifactregistry.repositories.get,run.services.create,run.services.get,run.services.update,run.services.setIamPolicy,iam.serviceAccounts.create,iam.serviceAccounts.get"
fi
